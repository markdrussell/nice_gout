	...
	foreach outcome in `outcomes' {
		...
		***Identify first observed proportion
		...
		if r(N) == 0 {
			...
			****Generate time variable from month 0
		...
		foreach demog_var in $demographic {
			...
			***Choose colour palette based on demographic variable (change as needed)
			if "`demog_var'" == "sex" {
				...
				local colours "sand forest_green ltblue ebblue dknavy orange cranberry red blue"
				local legtitle "Region"
			}
			else {
				local colours "sand forest_green ltblue ebblue dknavy dknavy orange cranberry red blue"
				local legtitle ""
			}
	...
	**Loop through outcomes of interest for full cohort
	foreach outcome in `outcomes' {
		...
		***Set x-axis format and title
		...
		forvalues y = `xmin_year'(1)`xmax_year' {
			local nextyear = mod(`y' + 1, 100)
			local nextyear_str : display %02.0f `nextyear'
			local xlabel `xlabel' `y' "`y'/`nextyear_str'"
		}
	...
	foreach outcome in `outcomes' {
		...
		**Loop through demographic variables of interest
		foreach demog_var in $demographic {
			...
			***Collapse monthly counts and denominators to year
			collapse (sum) count total, by(year outcome_name outcome_desc demog_group demog_level)

			***Recalculate annual proportion
			...
			***Set x-axis format and title
			quietly summarize year if !missing(prop), meanonly
			local xmin_year = r(min)
			local xmax_year = r(max)
			...
			***Extract variables of interest from data table
			...
			foreach subset of local demog_subset {
				...
				****Single plot per subset
				local thisplot connected prop year if demog_level==`"`subset'"', lcolor(`colour') lpattern(solid) mcolor(`colour'%30) msymbol(circle)
	...
	**Loop through outcome groups of interest
	foreach group in bloods comorbidities {
		...
		***Build plots
		...
		graph export "$projectdir/output/figures/temporal_plot_`table'_`group'.$img", replace width(5000)
	}
}
...
foreach table in ult_drug flares {
	...
	***Build plots
	...
	graph export "$projectdir/output/figures/temporal_plot_`table'_`outcome_group'.$img", replace width(5000)
	}
}
